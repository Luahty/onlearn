...
export default function Header() {
  ...
  const handleLogOut = async () => {
    try {
      ...
      throw error; // Hoặc xử lý lỗi theo yêu cầu của bạn
    }
  };
    ...
    <header className={`fixed w-full z-40 transition-all duration-300 ${scrolled ? 'bg-white shadow-lg h-20 xl:h-24 py-4' : 'bg-transparent h-32 py-8'}`}>
      <nav aria-label="Global" className="mx-auto flex max-w-full items-center justify-between px-6">
        ...
        <div className="hidden lg:flex lg:flex-1 lg:justify-end">
          {!user ? (<div onClick={handleLogOut} className={`${scrolled ? ' border-2 border-[#0B7077] hover:bg-teal-500 hover:border-teal-500' : 'text-[#0B7077] bg-white hover:bg-gray-100'} w-28 h-12 xl:w-[135px] xl:h-[59px] my-auto rounded-md text-center lg:py-3 xl:py-4 text-base font-semibold leading-6`}>LOG OUT</div>) :
            (<div className="hidden lg:flex lg:flex-1 lg:justify-end">
              ...
              <a href="/signup" className="w-28 h-12 xl:w-[135px] xl:h-[59px] my-auto ml-5 bg-[#0B7077] hover:bg-teal-500 rounded-md text-center lg:py-3 xl:py-4 text-base font-semibold leading-6 text-white">
                SIGN UP
              </a>
            </div>
            )}
